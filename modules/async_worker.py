...
def worker():
    ...
    def prepare_upscale(async_task,
                        goals,
                        uov_input_image,
                        uov_method,
                        performance,
    ...
    def apply_image_input(
        ...
        if async_task.current_tab == "inpaint" or (
            ...
            def to_uint8_array(x):
                ...
                # numpy array → assume already uint8 HWC
                return x

            # 1) Legacy Fooocus 3.x “sketch” dict
            if isinstance(raw, dict) and "image" in raw:
                inpaint_img = to_uint8_array(raw["image"])
                inpaint_msk = None if raw.get("mask") is None else to_uint8_array(raw["mask"])
            # 2) Gradio 5 ImageEditor dict
            elif isinstance(raw, dict) and "background" in raw and "composite" in raw:
                bg = to_uint8_array(raw["background"])
                comp = to_uint8_array(raw["composite"])
                diff = (comp.astype(int) != bg.astype(int)).any(axis=2)
                inpaint_img = bg
                inpaint_msk = (diff.astype(np.uint8) * 255)
            # 3) Fallback: path, array or PIL
            else:
                inpaint_img = to_uint8_array(raw)
                inpaint_msk = None

            # — Normalize to H×W×3 uint8 —
            if inpaint_img.ndim == 2:  # grayscale
                inpaint_img = np.stack([inpaint_img] * 3, axis=2)
            elif inpaint_img.ndim == 3 and inpaint_img.shape[2] == 4:  # RGBA
                inpaint_img = inpaint_img[..., :3]

            # — Normalize mask to H×W single-channel —
            if inpaint_msk is not None:
                if inpaint_msk.ndim == 3:
                    inpaint_msk = inpaint_msk[..., 0]

            # — Advanced-masking merge (unchanged) —
            if async_task.inpaint_advanced_masking_checkbox:
                mask_up = async_task.inpaint_mask_image_upload
                if isinstance(mask_up, dict) and "image" in mask_up and "mask" in mask_up:
                    img_arr = mask_up["image"]
                    mask_arr = mask_up["mask"]
                    if (
                        isinstance(img_arr, np.ndarray)
                        and isinstance(mask_arr, np.ndarray)
                        and img_arr.ndim == 3
                    ):
                        async_task.inpaint_mask_image_upload = np.maximum(img_arr, mask_arr)
                elif isinstance(mask_up, np.ndarray) and mask_up.ndim == 3:
                    async_task.inpaint_mask_image_upload = mask_up

            # — Morphological ops —
            if (
                inpaint_msk is not None
                and int(async_task.inpaint_erode_or_dilate) != 0
            ):
                inpaint_msk = erode_or_dilate(
                    inpaint_msk, async_task.inpaint_erode_or_dilate
                )
            if async_task.invert_mask_checkbox and inpaint_msk is not None:
                inpaint_msk = 255 - inpaint_msk

            inpaint_image = inpaint_img
            inpaint_mask = inpaint_msk

            # — Only download & queue an inpaint step if mask has “ink” or outpaint selected —
            if (
                ...
                if inpaint_parameterized:
                    ...
                    base_model_additional_loras.append(
        ...
        # — Control-Net / Enhance branches unchanged —
        if (
